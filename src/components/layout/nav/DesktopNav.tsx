    ...
    <div className="hidden md:block">
      <NavigationMenu>
        <NavigationMenuList className="space-x-2">
          {navigation.map(item => {
            // Skip rendering Admin item here since we'll render its children separately
            if (item.name === 'Admin') {
              return null;
            }
          ...
          {/* Render Admin buttons separately in the correct order */}
          {navigation.find(item => item.name === 'Admin')?.children?.map(child => (
